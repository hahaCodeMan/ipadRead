...
@synthesize parent = mpParent;
@synthesize children = mpChildren;
@synthesize key = mpstrKey;
@synthesize leafvalue = mpstrLeafValue;
@synthesize attributeDict = mpAttributeDict;
...
- (BOOL) isLeaf
{
	return (!self.children.count)?TRUE:FALSE;
}

- (BOOL) hasLeafValue
{
	return (self.leafvalue)?TRUE:FALSE;
}
...
- (NSArray *) uniqArray: (NSArray *) anArray
{
    ...
    {
		if (![[array lastObject] isEqualToString:object])
        {
...
- (void) dumpAtIndent: (int) indent into:(NSMutableString *) outstring
{
	...
	if (self.leafvalue)
    {
        [outstring appendFormat:@"(%@)", STRIP(self.leafvalue)];
    }
	[outstring appendString:@"\n"];
	
	for (TreeNode *node in self.children)
    {
        [node dumpAtIndent:indent + 1 into: outstring];
    }
}
...
- (NSMutableDictionary *) dictionaryForChildren
{
	...
	for (TreeNode *node in self.children)
    {
        ...
        {
            [results setObject:node.leafvalue forKey:node.key];
        }
	}
	return results;
}
...
{ 
	if ([self.children respondsToSelector:sel])
    {
...
- (BOOL)respondsToSelector:(SEL)aSelector
{
	...
	if ([self.children respondsToSelector:aSelector]) 
    {
        return YES;
    }
	return NO;
...
- (BOOL)isKindOfClass:(Class)aClass
{
	...
	if ([self.children isKindOfClass:aClass])
    {
        return YES;
    }
	return NO;
